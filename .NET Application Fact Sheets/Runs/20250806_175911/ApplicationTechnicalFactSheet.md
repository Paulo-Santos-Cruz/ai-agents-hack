...
This section identifies all distinct types of users (or autonomous systems) that interact with the application. These roles are inferred from authentication logic, authorization checks (e.g., `ConfirmaPrivilegios` method), or conditional logic based on user properties.
...
| :--- | :--- | :--- |
| **Architecture pattern** | **N-Tier, Monolithic** | The application is structured into logical layers (Presentation, Business, Data Access) but is deployed as a single, cohesive set of executables and libraries. There is no evidence of microservices or a distributed architecture. |
| **Architectural patterns** | **Model-View-Controller (MVC) - variant** | The WinForms code exhibits a separation between the Form (View), event handlers (Controller), and data-handling classes (Model-like objects, e.g., `DetalheRemessa`), though it is not a strict, modern MVC implementation. |
...
| Category | Technology / Library |
| :--- | :--- |
| **Database management system(s)** | Microsoft SQL Server 2008 R2 (inferred from server names like `S2K081`) |
...
| **Database authentication** | Windows Authentication or SQL Server Authentication (inferred) |
...
| **Ciclomatic complexity** | Measures the number of linearly independent paths through a program's source code. Higher values indicate more complex code that is harder to test and maintain. | < 10: Low risk<br>11-20: Moderate risk<br>21-50: High risk<br>> 50: Very high risk |
...
| :--- | :--- | :--- |
| **Windows Application** | An executable project that runs as a desktop application or a service. | 2 |
| **Class Library** | A reusable library of code (.dll) that cannot be executed on its own. | 14 |
| **Setup Project** | A project for creating a Windows Installer (.msi) package. | 1 |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **GenericNet** | A generic utility library, likely for database access and configuration management. | Library | `GenericNet.dll` | 2.0.0.1 | v2.0.50727 | Internal | Proprietary | Core application functions, data access. |
| **NBIISNET** | A custom Windows Forms component library. | Library | `NBIISNET.dll` | 2.0.5.2 | v2.0.50727 | Internal | Proprietary | Custom UI controls like `ListViewBase`. |
| **GenericLogNET** | A generic logging library. | Library | `GenericLogNET.dll` | 2.0.0.1 | v2.0.50727 | Internal | Proprietary | Logging errors, warnings, and info. |
...
| **CGDSendWebmail** | A library for sending emails, likely specific to the organization. | Library | `CGDSendWebmail.dll` | 1.0.0.0 | v2.0.50727 | Internal | Proprietary | Sending alert notifications. |
...
| :--- | :--- | :--- | :--- | :--- |
| N/A | N/A | N/A | N/A | The project predates the widespread adoption of NuGet. All dependencies are referenced as local files or from a UNC path. |
...
| **ASP.NET Membership Provider in use** | ❌ Not used. The application is a desktop/service application and uses its own user store. |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| SEC-001 | A01: Broken Access Control | Decentralized and inconsistent authorization checks. | Authorization is scattered in `if (user.group > 1)` checks throughout the UI code, making it easy to miss a check and introduce a vulnerability. | `ActividadeBalcaoForm.cs`, `UtilizadoresForm.cs` | 🔴 High | 🔴 High | 🔴 High | Implement centralized, attribute-based authorization. | [OWASP A01](https://owasp.org/Top10/A01_2021-Broken_Access_Control/) |
| SEC-003 | A03: Injection | Critical SQL Injection risk from string concatenation. | Raw SQL queries are constructed by concatenating strings with variables (e.g., `sQuery += " where ALERT_ID=" + m_sALERT_ID;`). | `AlertaSituacaoAccao.cs`, `PesquisasForm.cs` | 🔴 High | 🔴 Critical | 🔴 Critical | Use parameterized queries or an ORM like Entity Framework Core exclusively. | [OWASP A03](https://owasp.org/Top10/A03_2021-Injection/) |
| SEC-005 | A06: Vulnerable & Outdated Components | Use of End-of-Life .NET Framework 4.0. | The application targets **.NET Framework 4.0**, which has been out of support since January 2016 and no longer receives security patches. | All `.csproj` files | 🔴 High | 🔴 Critical | 🔴 High | Migrate to a supported .NET version (e.g., .NET 8). | [.NET Framework Lifecycle](https://dotnet.microsoft.com/en-us/platform/support/policy/dotnet-framework) |
| SEC-008 | A08: Software & Data Integrity Failures | Unsafe Deserialization Risk. | Use of an old, unpatched .NET framework can be susceptible to deserialization vulnerabilities if data is exchanged with external sources. | Core Framework | 🟠 Medium | 🔴 High | 🟠 Medium | Upgrade the framework and audit all data deserialization points. | [OWASP A08](https://owasp.org/Top10/A08_2021-Software_and_Data_Integrity_Failures/) |
| SEC-009 | A09: Security Logging and Monitoring Failures | Inadequate logging and empty catch blocks. | Several `try-catch` blocks are empty (`catch {}`), which swallows exceptions without logging, preventing detection of attacks or errors. | `AlertaSituacaoAccao.cs`, `ServAlerta.cs` | 🟠 Medium | 🟠 Medium | 🟠 Medium | Implement centralized exception handling and ensure all exceptions are logged. | [OWASP A09](https://owasp.org/Top10/A09_2021-Security_Logging_and_Monitoring_Failures/) |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| SEC-013 | Configuration | Secrets stored in config files. | Database server and name details are stored in plain text within `.config` files, increasing risk of information disclosure. | All `.config` files | 🔴 High | 🔴 High | 🔴 High | Use a secure secret management system like Azure Key Vault. | [App Secrets](https://learn.microsoft.com/en-us/aspnet/core/security/app-secrets) |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| Exception Handling | Empty `catch` blocks | Centralized exception handling | `CIServico`, `Alerta` projects | 🟠 Medium | 🔴 High | 🟠 Medium | Implement a global exception handler and robust logging. | [Exception Handling](https://learn.microsoft.com/en-us/dotnet/standard/exceptions/) |
| Configuration | Hardcoded file paths | Externalize configuration | `CIFicheiro`, `.config` files | 🟠 Medium | 🟠 Medium | 🟠 Medium | Move all paths to a single, managed configuration source. | [Configuration in .NET](https://learn.microsoft.com/en-us/dotnet/core/extensions/configuration) |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| .NET Framework | 4.0 | Multiple (unsupported) | Microsoft | 🔴 High | 🔴 Critical | 🔴 High | Upgrade to .NET 8 or .NET Framework 4.8.1. | [NVD](https://nvd.nist.gov/) |
| Crystal Reports for VS 2010 | 13.0.x | Multiple (unsupported) | SAP | 🟠 Medium | 🔴 High | 🟠 Medium | Upgrade to a supported version or replace with a modern reporting tool. | [SAP Notes](https://support.sap.com/en/index.html) |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `.config` files | Hardcoded server names | Configuration is not portable and leaks infrastructure details. | 🟠 Medium | 🟠 Medium | 🔴 High | Externalize connection strings and server names. | [Configuration](https://learn.microsoft.com/en-us/dotnet/core/extensions/configuration) |
| `app.config` | No `customErrors` mode | By default, detailed error messages could be shown, leaking information. | 🟢 Low | 🟠 Medium | 🟢 Low | Explicitly configure error handling behavior. | [customErrors](https://learn.microsoft.com/en-us/dotnet/framework/configure-apps/file-schema/web/customerrors-element) |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| SEC-016 | Secrets | Hardcoded Connection Details | Database host and name are in config files. | `*.config` | 🔴 High | 🔴 High | 🔴 High | Use environment variables or a secret store. | [Secrets Management](https://learn.microsoft.com/en-us/aspnet/core/security/key-vault-configuration) |
| SEC-017 | Logging | Potential Sensitive Data in Logs | Exception details logged might contain sensitive business data. | `GenericLogNET` calls | 🟠 Medium | 🟠 Medium | 🟠 Medium | Review and sanitize all logged data to prevent PII leakage. | [Logging Best Practices](https://owasp.org/www-project-proactive-controls/v3/en/c7-enforce-logging-monitoring) |
| SEC-018 | Crypto | Inferred Weak Password Storage | No evidence of modern hashing algorithms like PBKDF2 or Argon2. | `User.cs`, Database | 🔴 High | 🔴 High | 🔴 High | If passwords are stored, ensure they are hashed with a salt using a strong algorithm. | [Password Storage](https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html) |
...
| **Web Service** | **MDI Document Insertion** | A SOAP-based web service used to insert MDI (Multi-function Device Interface) documents into a remote system. | `http://waiaccesstu:80/UWTDWeb/services/Insert_DocumentoMDIService` | None apparent | XML (SOAP) | HTTP |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Integration Testing** | MSTest | N/A | ❔ Unknown | The `CITestes` project contains tests that instantiate business logic classes and interact with the database, which is characteristic of integration tests. | ❔ Unknown | ❔ Unknown |
| **Unit Testing** | MSTest | N/A | ❔ Unknown | While a test project exists, the provided tests act more as test harnesses to run processes rather than isolated unit tests with mocks and assertions. | ❔ Unknown | ❔ Unknown |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| ISSUE-001 | End-of-Life .NET Framework | Legacy | Limitation | 🔴 High | 🔴 Critical | 🔴 High | All Projects | Migrate to a supported .NET version (e.g., .NET 8). |
| ISSUE-002 | End-of-Life SQL Server | Legacy | Limitation | 🔴 High | 🔴 Critical | 🔴 High | Database | Migrate to a supported SQL Server version (e.g., SQL Server 2022). |
| ISSUE-003 | Lack of Horizontal Scalability | Architecture | Limitation | 🟠 Medium | 🔴 High | 🟠 Medium | `CIServico` | Refactor the application to a cloud-native architecture (e.g., Azure Functions). |
| ISSUE-004 | Tightly Coupled Code | Code Quality | Defect | 🟠 Medium | 🟠 Medium | 🟠 Medium | UI and Business Logic | Introduce Dependency Injection and interfaces to decouple components. |
| ISSUE-005 | No Centralized Configuration | Configuration | Defect | 🟠 Medium | 🟠 Medium | 🟠 Medium | All Projects | Consolidate configuration into a centralized source like Azure App Configuration. |
...
| ISSUE-007 | Hardcoded Environment Details | Configuration | Defect | 🔴 High | 🔴 High | 🔴 High | `.config` files | Externalize all environment-specific settings. |
| ISSUE-008 | Poor Exception Handling (Empty Catch) | Code Quality | Defect | 🟠 Medium | 🔴 High | 🟠 Medium | Various classes | Implement a global exception handler and remove all empty `catch` blocks. |
| ISSUE-009 | No Internationalization Support | UI | Limitation | 🟢 Low | 🟢 Low | 🟢 Low | WinForms projects | Externalize all UI strings into resource files (`.resx`). |
| ISSUE-010 | Outdated UI Technology (WinForms) | Legacy | Limitation | 🟠 Medium | 🟠 Medium | 🟠 Medium | `CIActividades`, `CIControlo` | Migrate the UI to a modern platform like Blazor or MAUI. |
| ISSUE-011 | Direct UNC Path Dependencies | Configuration | Defect | 🟠 Medium | 🟠 Medium | 🟠 Medium | `.csproj` files | Replace UNC path references with a package management solution like a private NuGet feed. |
| ISSUE-012 | Manual Thread Management | Performance | Defect | 🟠 Medium | 🟠 Medium | 🟠 Medium | `CIServicoThread.cs` | Replace manual thread loops with modern async/await patterns and TPL. |
| ISSUE-013 | Use of `DataSet` for Data Transfer | Code Quality | Defect | 🟠 Medium | 🟠 Medium | 🟠 Medium | `CIConfiguration`, `CIActividades` | Replace `DataSet` with strongly-typed DTOs for better type safety. |
| ISSUE-014 | No `async`/`await` for I/O | Performance | Defect | 🟠 Medium | 🟠 Medium | 🟠 Medium | Data Access and File I/O code | Refactor all synchronous I/O operations to be asynchronous. |
| ISSUE-015 | Obsolete Reporting Technology | Legacy | Limitation | 🟠 Medium | 🟠 Medium | 🟠 Medium | `CIReports` | Replace Crystal Reports with a modern solution like Power BI or SSRS. |
| ISSUE-016 | Lack of Test Automation | Testing | Limitation | 🟠 Medium | 🟠 Medium | 🟠 Medium | `CITestes` | Implement a full suite of automated unit, integration, and UI tests. |
| ISSUE-017 | No Health Check Endpoints | Supportability | Limitation | 🟠 Medium | 🟢 Low | 🟠 Medium | `CIServico` | Implement health check logic for monitoring application status. |
| ISSUE-018 | Monolithic Database Schema | Architecture | Limitation | 🟠 Medium | 🟠 Medium | 🟠 Medium | Database | Decompose the database if the application is refactored into microservices. |
| ISSUE-019 | Inconsistent Naming Conventions | Code Quality | Defect | 🟢 Low | 🟢 Low | 🟢 Low | All Projects | Refactor code to adhere to standard .NET naming conventions. |
| ISSUE-020 | High Cyclomatic Complexity in UI | Code Quality | Defect | 🟠 Medium | 🟠 Medium | 🟠 Medium | `ActividadesForm.cs` | Refactor large methods in UI forms into smaller, more manageable pieces. |
| ... | ... | ... | ... | ... | ... | ... | ... | ... |
| ISSUE-050 | Business Logic in UI Layer | Architecture | Defect | 🟠 Medium | 🟠 Medium | 🟠 Medium | `ActividadesForm.cs` | Move business logic from UI event handlers to a dedicated service layer. |
...
| DEF-004 | Use of `DataSet` for Data Transfer | Code Quality | Defect | 🟠 Medium | 🟠 Medium | 🟠 Medium | Legacy Practices | `CIConfiguration`, `CIActividades` | Replace `DataSet` with strongly-typed DTOs for better type safety and maintainability. |
...
| VULN-008 | **Lack of Input Validation** | Injection | Bug | 🟠 Medium | 🟠 Medium | 🟠 Medium | Trusting user input | `PesquisasForm.cs` | Validate all user-provided input on the server-side before using it in queries or logic. |
| VULN-009 | **Weak Authorization Model** | Broken Access Control | Defect | 🔴 High | 🔴 High | 🔴 High | Simple integer-based groups | `User.cs`, `ActividadeBalcaoForm.cs` | Migrate to a claims-based or policy-based authorization model for fine-grained control. |
| VULN-010 | **Reliance on Obsolete `System.Web.Services`** | Vulnerable Components | Legacy | 🟠 Medium | 🟠 Medium | 🟠 Medium | Outdated Technology | `MDIWebTransmCI.vbproj` | Replace the SOAP client with a modern `HttpClient`-based client. |
| ... | ... | ... | ... | ... | ... | ... | ... | ... | ... |
| VULN-050 | **Uncontrolled Resource Consumption** | Denial of Service | Limitation | 🟠 Medium | 🟠 Medium | 🟠 Medium | No rate limiting | `CIServico` | Implement throttling on file processing to prevent resource exhaustion from a flood of input files. |
...
| Windows Server 2012 | 2012-10-30 | 2018-10-09 | 2023-10-10 | ❌ Not supported | ❔ Likely |
| Windows Server 2008 R2 | 2009-10-22 | 2015-01-13 | 2020-01-14 | ❌ Not supported | ❔ Likely |
| Windows Server 2008 | 2008-05-06 | 2015-01-13 | 2020-01-14 | ❌ Not supported | ❔ Likely |
...
| SQL Server 2008 | 2008-11-06 | 2014-07-08 | 2019-07-09 | ❌ Not supported | ❔ Likely |
...
| **Lift and Shift (Rehost to Cloud)** | Upgrade to .NET Framework 4.8.1 and migrate the application and database to Azure VM and Azure SQL. | 🟠 Medium | 3-5 weeks | 2-3 weeks | 3-4 weeks | 1-2 weeks | 9-14 weeks | 🟠 Medium | 🟠 Medium | 🟠 Medium | 🟠 Medium | ✅ Introduces cloud benefits (IaaS)<br>✅ Improves reliability and manageability<br>✅ Step towards further modernization | ❌ Still on legacy .NET Framework<br>❌ Does not fully leverage cloud-native features<br>❌ Higher operational cost than on-prem |
| **Complete Refactor (Cloud-Native)** | Rewrite the application to target .NET 8. Decompose into a Blazor UI and a backend Worker Service or Azure Functions. Use EF Core. | 🔴 High | 9-12 weeks | 6-8 weeks | 4-6 weeks | 2-3 weeks | 21-29 weeks | 🔴 High | 🔴 High | 🟢 High | 🔴 High | ✅ Addresses all technical debt<br>✅ Fully supported and secure stack<br>✅ Cloud-native, scalable, and maintainable<br>✅ Best long-term value | ❌ Highest cost and effort<br>❌ Highest risk of functional regressions<br>❌ Longest time-to-market |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Programming Language** | C# | Programming Language | 4.0 (VS 2010) | Microsoft | 2010-04-12 | 2015-07-14 | 2020-07-14 | ❌ Unsupported |
| **Programming Language** | VB.NET | Programming Language | 10.0 (VS 2010) | Microsoft | 2010-04-12 | 2015-07-14 | 2020-07-14 | ❌ Unsupported |
